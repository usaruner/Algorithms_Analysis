#include <vector>

//REQUIREMENTS:
//each sort should be implemented in its own file of the same name e.g. insertion_sort.cpp
//each file should #include this header file
//no file should use anything outside of the C++ standard library
//functions should be tested using g++ in a linux environment
//each function should modify the input vector so that it is sorted upon completion
#ifndef PROJECT1_H
#define PROJECT1_H
void printVec(std::vector<int>& nums);
void insertion_sort(std::vector<int>& nums);
void merge_sort(std::vector<int>& nums);
void shell_sort1(std::vector<int>& nums, int i);
void shell_sort2(std::vector<int>& nums, int i);
void shell_sort3(std::vector<int>& nums, int i);
void shell_sort4(std::vector<int>& nums, int i);
void hybrid_sort1(std::vector<int>& nums);
void hybrid_sort2(std::vector<int>& nums);
void hybrid_sort3(std::vector<int>& nums);

#endif 
