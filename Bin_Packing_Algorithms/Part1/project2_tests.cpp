...
void testAlgorithm(ProblemInstance test, ProblemInstance expected_result, algorithm algo, std::string name)
{
	...
	for (int i = 0; i < (int)test.free_space.size(); i++)
	{
		std::cout << test.free_space.at(i) << ' ';
	}
	std::cout << std::endl;
	std::cout << "The expected vector elements are : ";

	for (int i = 0; i < (int)expected_result.free_space.size(); i++)
	{
		std::cout << expected_result.free_space.at(i) << ' ';
	}
	std::cout << std::endl;
	std::cout << "The vector elements are : ";

	for (int i = 0; i < (int)test.assignments.size(); i++)
	{
		std::cout << test.assignments.at(i) << ' ';
	}
	std::cout << std::endl;
	std::cout << "The expected vector elements are : ";

	for (int i = 0; i < (int)expected_result.assignments.size(); i++)
	{
		std::cout << expected_result.assignments.at(i) << ' ';
	}
	std::cout << std::endl;
...
int main()
{
	...
	for (int i = 0; i < 1000; i++) {
		items.push_back(((double)rand()/ RAND_MAX));
	}
	assignments = std::vector<int>(items.size(), 0);
	...
	std::vector<double> expected = std::vector<double>((int)items.size(), 1);
	std::vector<int> buck = std::vector<int>((int)items.size());
	
	int j = 0;
	for (int i = 0; i < (int)items.size(); i++) {
		j = 0;
		std::sort(expected.begin(), expected.end());
		while (temp[i] >= expected[j]) {
			j++;
		}
		buck[i] = j;
		expected[j] -= temp[i];
		...
		{
			std::cout << expected.at(k) << ' ';
		}
		std::cout << std::endl;
	}
	if ((int)expected.size() > 3) {
		while (*(expected.end() - 1) == 1) {
			expected.pop_back();
		}
	}
